...
namespace Research.Web.Framework.TagHelpers.Shared
{
    ...
    public class ResearchDatePickerTagHelper : TagHelper
    {
        ...
        public ResearchDatePickerTagHelper(IHtmlGenerator generator, IHtmlHelper htmlHelper)
        {
            Generator = generator;
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            days.AppendFormat("<option value='{0}'>{1}</option>", "0", "วัน");
            for (var i = 1; i <= 31; i++)
                days.AppendFormat("<option value='{0}'{1}>{0}</option>", i, 
                    (SelectedDay.HasValue && SelectedDay.Value == i) ? " selected=\"selected\"" : null);

            months.AppendFormat("<option value='{0}'>{1}</option>", "0", "เดือน");
            for (var i = 1; i <= 12; i++)
            {
