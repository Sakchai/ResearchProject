...
namespace Research.Web.Validators
{
    ...
    public abstract class BaseResearchValidator<TModel> : AbstractValidator<TModel> where TModel : class
    {
        #region Ctor

        protected BaseResearchValidator()
        {
            PostInitialize();
        }
           ...
           // var localizationService = EngineContext.Current.Resolve<ILocalizationService>();
            foreach (var expression in maxValueExpressions)
            {
                RuleFor(expression.Expression).IsDecimal(expression.MaxValue)
                    .WithMessage(string.Format("Max value {0} ", expression.MaxValue - 1));
            }
        }
